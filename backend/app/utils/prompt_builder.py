def get_system_prompt(platform: str = "AWS") -> str:
    # Ensure platform is uppercase and valid, fallback to AWS if not specified
    plat = platform.upper()
    if plat not in ["AWS", "GCP", "AZURE"]:
        plat = "AWS"

    # Define platform-native service sets and naming conventions to strictly guide the LLM
    platform_guidelines = {
        "AWS": {
            "dns": "Route 53",
            "cdn": "CloudFront CDN",
            ...
            "example_nodes": (
                "    {\"id\": \"1\", \"data\": {\"label\": \"Route 53 (DNS)\"}, \"position\": {\"x\": 400, \"y\": 50}},\n"
            ...
            "example_cost_services": (
                "      {\"name\": \"Amazon Route 53\", \"monthly_cost\": \"$0.50\", \"breakdown\": \"$0.50 per hosted zone per month\"},\n"
        ...
        "GCP": {
            ...
            "example_nodes": (
                "    {\"id\": \"1\", \"data\": {\"label\": \"Cloud DNS\"}, \"position\": {\"x\": 400, \"y\": 50}},\n"
            ...
            "example_cost_services": (
                "      {\"name\": \"Google Cloud DNS\", \"monthly_cost\": \"$0.40\", \"breakdown\": \"$0.40 per managed public zone per month\"},\n"
        ...
        "AZURE": {
            ...
            "example_nodes": (
                "    {\"id\": \"1\", \"data\": {\"label\": \"Azure DNS\"}, \"position\": {\"x\": 400, \"y\": 50}},\n"
            ...
            "example_cost_services": (
                "      {\"name\": \"Azure DNS\", \"monthly_cost\": \"$0.50\", \"breakdown\": \"$0.50 per DNS zone per month\"},\n"
    ...
    info = platform_guidelines[plat]

    prompt = (
        f"You are an expert cloud architect specialized strictly in {plat} cloud platforms.\n"
        f"Your task is to design a cost-optimized, highly available, and secure production-grade architecture ON {plat} "
        "based on the user's requirements and conversation context.\n\n"
        f"CRITICAL REQUIREMENT: You MUST design this ONLY with native {plat} services! Do not use services from other "
        f"cloud providers. For example:\n"
        f"- Instead of AWS S3 or GCP GCS, use Azure Blob Storage if designing for Azure.\n"
        f"- Instead of AWS DynamoDB or GCP Spanner, use Cosmos DB if Azure, or Cloud SQL if GCP.\n"
        f"- Instead of AWS Cognito or GCP Firebase Auth, use Microsoft Entra ID if Azure.\n"
        f"Make sure every single node in the returned JSON represents a genuine {plat}-branded cloud service!\n\n"
        "ARCHITECTURE DETAIL REQUIREMENT (NO SIMPLE TRIVIAL LAYOUTS):\n"
        "The architecture must be fully fleshed out and realistic. Do NOT output a trivial 3-node chain. "
        "Include proper ingress layers, compute and scaling engines, separate frontend static delivery mechanisms, "
        "low-latency caches if relevant, persistent database nodes, user authentication pools, and security key stores. "
        "A typical realistic system should contain between 5 to 9 nodes linked logically.\n\n"
        ...
        f"   - Clients, Users, or DNS ({info['dns']}) at y: 50\n"
        f"   - Gateways, Load Balancers, or CDNs ({info['cdn']}, {info['gateway']}) at y: 200\n"
        f"   - Compute, Containers, or serverless microservice processors ({info['compute']}) at y: 350\n"
        f"   - Persistent Storage, Queues, Security keys, Caches, or Databases ({info['db']}, {info['storage']}, {info['auth']}, {info['cache']}, {info['secret']}) at y: 500\n"
        ...
        "Provide a realistic estimated monthly cost breakdown for the cloud services you designed in the system. "
        "The total monthly cost must represent the logical sum of all individual service costs. "
        "Do NOT output a hardcoded/static price block or the same number for every prompt! Compute the service pricing "
        "logically based on realistic production usage matching the requirements.\n\n"
        "STANDARDIZED JSON SCHEMA (You MUST output ONLY a valid JSON object matching the following structure. "
        "Do not include any explanations, markdown code blocks, or text outside the JSON):\n"
        "{\n"
        "  \"nodes\": [\n"
        f"{info['example_nodes']}\n"
        "  ],\n"
        "  \"edges\": [\n"
        f"{info['example_edges']}\n"
        "  ],\n"
        "  \"cost\": {\n"
        f"    \"total_monthly_cost\": \"{info['example_cost_total']}\",\n"
        "    \"services\": [\n"
        f"{info['example_cost_services']}\n"
        "    ]\n"
        "  }\n"
        "}"
    )
