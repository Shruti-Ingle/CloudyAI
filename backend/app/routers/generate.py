...
def generate_architecture(req: GenerateRequest, current_user: dict = Depends(get_current_user)):
    def _run_generation():
        # 1. Try Ollama first!
        try:
    ...
    result_payload = _run_generation()
    ...
    if result_payload.get("status") == "success":
        try:
            ...
            s3_key = f"generations/{user_id}/{generation_id}.json"
            json_bytes = json.dumps(result_payload, indent=2).encode('utf-8')
            s3_success = s3_service.upload_file(json_bytes, s3_key)
            if s3_success:
                print(f"Successfully uploaded architecture to S3: {s3_key}")
            else:
                print("Warning: S3 upload failed, proceeding anyway")
                
            # 2. Save metadata + S3 key in DynamoDB
            dynamo_service = DynamoService()
            
            # Format data to store in DynamoDB
            db_data = {
            ...
            dynamo_success = dynamo_service.save_generation(
                user_id=user_id,
                title=req.prompt,
                platform=req.platform,
                data=db_data
            ...
            if dynamo_success:
                print(f"Successfully logged generation in DynamoDB for user {user_id}")
            else:
                print("Warning: DynamoDB log failed")
                
        except Exception as aws_err:
            print(f"Failed to perform AWS background saves: {aws_err}")
...
def get_history(current_user: dict = Depends(get_current_user)):
    try:
        ...
        dynamo_service = DynamoService()
        user_id = current_user.get("sub", "anonymous")
        db_items = dynamo_service.get_user_history(user_id)
        ...
        for item in db_items:
            ...
            # Map database keys to format expected by frontend
            history_list.append({
                "id": data.get("generation_id", item.get("SK", "")),
                "type": "generated",
                "title": item.get("title", "Untitled Architecture"),
                "platform": item.get("platform", "AWS"),
                "services": len(nodes),
                "cost": cost.get("total_monthly_cost", "$0.00"),
                "nodes": nodes,
        ...
        # Fall back to empty history in case of db query errors
        return []
